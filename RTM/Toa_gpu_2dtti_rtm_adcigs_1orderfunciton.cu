...
/************************i****************************************/
            if(ix>=mm&&ix<(nnx-mm)&&iz>=mm&&iz<(nnz-mm))
            {
                  ...
                  px1[id]=acoffx2[ix]*acoffz2[iz]*px0[id] - acoffx1[ix]*acoffz1[iz]*vp[id]*vp[id]*(1+2*ee)*(cos(theta[id])*dtx*ux-sin(theta[id])*dtz*uz);
                  pz1[id]=acoffx2[ix]*acoffz2[iz]*pz0[id] - acoffx1[ix]*acoffz1[iz]*vp[id]*vp[id]*sqrtf(1+2*dd)*(sin(theta[id])*dtx*wx+cos(theta[id])*dtz*wz);
                  qx1[id]=acoffx2[ix]*acoffz2[iz]*qx0[id] - acoffx1[ix]*acoffz1[iz]*vp[id]*vp[id]*sqrtf(1+2*dd)*(cos(theta[id])*dtx*ux-sin(theta[id])*dtz*uz);
                  qz1[id]=acoffx2[ix]*acoffz2[iz]*qz0[id] - acoffx1[ix]*acoffz1[iz]*vp[id]*vp[id]*(sin(theta[id])*dtx*wx+cos(theta[id])*dtz*wz);
                  P[id]=px1[id]+pz1[id];
                  Q[id]=qx1[id]+qz1[id];
            }
      }
...
/*************func**************/    
__global__ void mute_truedirectwave(int nx,int nt,float *dir,float *shot)
{
int id=threadIdx.x+blockDim.x*blockIdx.x;

if(id<nx*nt)
{
shot[id]=shot[id]-dir[id];
}
}
...
/*************func**************/ 
__global__ void Poynting_migration(int nnx, int nnz, int nx, int nz, int npml, int na, int da,
                        float *s_P, float *s_Q, float *s_u, float *s_w,
                        float *g_P, float *g_Q, float *g_u, float *g_w, float *migration)
{
      int id=threadIdx.x+blockDim.x*blockIdx.x;
      int ix, iz;
      float Ssx,Ssz,Sgx,Sgz,b1,b2,a,b,c;
      int ia;
      ...
      if(id<nz*nx){ 
            ...
            ix=id/nz;
            iz=id%nz;      
            Ssx=-s_P[iz+npml+nnz*(ix+npml)]*s_u[iz+npml+nnz*(ix+npml)];
            Ssz=-s_Q[iz+npml+nnz*(ix+npml)]*s_w[iz+npml+nnz*(ix+npml)];
            Sgx= g_P[iz+npml+nnz*(ix+npml)]*g_u[iz+npml+nnz*(ix+npml)];
            Sgz= g_Q[iz+npml+nnz*(ix+npml)]*g_w[iz+npml+nnz*(ix+npml)];
            b1= Ssx*Ssx + Ssz*Ssz;
            b2= Sgx*Sgx + Sgz*Sgz;
            a=(Ssx*Sgx + Ssz*Sgz)/(sqrtf(b1*b2)*(1-0.1));                    
              
            //if(a>0){
                  a=0.5*acosf(a)*180.0/pi;
                  ia=(int)(a/(da*1.0));
                  //if(ia<90){
                        migration[id]+=s_P[iz+npml+nnz*(ix+npml)]*g_P[iz+npml+nnz*(ix+npml)];
                  //}
            //}            
      }
}
                           ...
                           float *g_P, float *g_Q, float *g_u, float *g_w)
{
    ...
    float b =(Ssx*Sgz - Sgx*Ssz);//(sqrtf(b1*b2)*(1 - 0.1));
   ...
   if(id<nx/dcdp*nz)
   {
     if(a>=-1&&a<=1)
      {
         //a=0.5*acosf(a)*180.0/pi;
	 b=0.5*asinf(a)*180.0/pi;
	 //if(a <= 60)
		
         //ia=(int)(a/(da*1.0));
         //if(ia<na)
          {
	     ia = (int)(b*na/180)+na/2;
...
int main(int argc,char *argv[])
{
	...
	float *g_P, *g_Q;
      float *shot_Dev, *shot_Hos, *dir_Hos, *s_P_bndr, *s_Q_bndr;
      float *dir_Dev;

      float *migration, *illumination, *adcigs;
      float *Atemp;

      float *ini_v, *ini_vp, *cons_v, *cons_vp;
	...
	char tmp[20],tmp1[20],tmp2[20];
      ...
      sprintf(tmp1,"%s", argv[2]);
      sprintf(tmp2,"%s", argv[3]);
      printf("No. %s velocity model, using the GPU %s, with FWI velocity %s\n",tmp,tmp1,tmp2);
      index=atoi(tmp); //将数字格式的字符串转换成整数类型 index————真实速度模型
      igpu=atoi(tmp1); // 使用的gpu id
      iter=atoi(tmp2); // 初始模型

      if(!iter)
            printf("migration with True velocity\n ");
      else
            printf("migration with FWI velocity\n ");
...
/********** dat document ***********/
      ...
      char FN3[520];
      char FN4[520];
      char FN5[250], FN6[250], FN7[250], FN8[250], FN9[250];
/********aaa************/  
...
/********* parameters *************/
      ...
      fscanf(fpsnap, "%d%d%f%f", &nx,&nz,&dx,&dz);printf("nx,dx,nz,dz=%d,%f,%d,%f\n",nx,dx,nz,dz);
      fscanf(fpsnap, "%d%f", &nt,&dt);printf("nt,dt=%d,%f\n",nt,dt);
      fscanf(fpsnap, "%f%f", &favg,&pfac);printf("favg,pfac=%f,%f\n", favg, pfac);
      fscanf(fpsnap, "%d%d%d", &fs,&ds,&ns);printf("fs,ds,ns=%d,%d,%d\n",fs,ds,ns);
      fclose(fpsnap);
...
/************a*************/
      ...
      dir_Hos =(float*)malloc(nt*nx*sizeof(float));
      read_file(FN1,FN2,FN3,FN4,nx,nz,nnx,nnz,v,e,d,b,npml);
      printf("read_file is ok\n");
/****************************/
      ...
      cudaSetDevice(igpu);// initialize device, default device=0;
...
/****************************/
      ...
      cudaMalloc(&dir_Dev, nx*nt*sizeof(float));
/******************************/
      check_gpu_error("Failed to allocate memory for variables!");
       ...
       if(1)
       {
            ...
            for(int ix=npml;ix<=nnx-npml-1;ix++)
           {
                 ...
                 {
                       fread(&v[ix*nnz+iz],sizeof(float),1,fpsnap);
                 }
           }
            fclose(fpsnap);
            pad_vv(nx,nz,nnx,nnz,npml,v); 
            sprintf(FN8,"output/ini_vp_No%d_Iter%d.dat",index,iter);
            fpsnap=fopen(FN8,"wb");
           //  fpsnap=fopen("./output/ini_vp.dat","wb");
           //  fwrite(v,sizeof(float),nnx*nnz,fpsnap);
     
            for(int ix=npml;ix<=nnx-npml-1;ix++)
            {
                  for(int iz=npml;iz<=nnz-npml-1;iz++)
                  {
                        fwrite(&v[ix*nnz+iz],sizeof(float),1,fpsnap);
                  }
            }
            fclose(fpsnap);
            
       }
      cudaMalloc(&ini_vp, nnz*nnx*sizeof(float));
      cudaMemcpy(ini_vp, v, nnz*nnx*sizeof(float), cudaMemcpyHostToDevice);
      ...
      for(i=npml;i<nx+npml;i++)
      {
            for(j=npml;j<nz+npml;j++)
            {
                  v[i*nnz+j]=v[i*nnz+npml];
            }
      }
       pad_vv(nx,nz,nnx,nnz,npml,v); 
       fpsnap=fopen("./output/const_vp.dat","wb");
      //  fwrite(v,sizeof(float),nnx*nnz,fpsnap);
      for(int ix=npml;ix<=nnx-npml-1;ix++)
      {
            for(int iz=npml;iz<=nnz-npml-1;iz++)
      ...
      fclose(fpsnap);
       ...
       cudaMalloc(&cons_vp, nnz*nnx*sizeof(float));
       cudaMemcpy(cons_vp, v, nnz*nnx*sizeof(float), cudaMemcpyHostToDevice);
       printf("The constant model is done!\n"); 
...
/**********IS Loop start*******/
      ...
      {   
            ...
            { 
                  add_source<<<1,1>>>(pfac,fs,zs,nx,nz,nnx,nnz,dt,t,favg,wtype,npml,is,ds,s_P,s_Q);
                  update_vel<<<(nnx*nnz+511)/512, 512>>>(nx,nz,nnx,nnz,npml,dt,dx,dz,s_u0,s_w0,s_u1,s_w1,s_P,s_Q,coffx1,coffx2,coffz1,coffz2,theta);
                  update_stress<<<(nnx*nnz+511)/512, 512>>>(nx,nz,nnx,nnz,dt,dx,dz,s_u1,s_w1,s_P,s_Q,ini_vp,npml,
                                                  s_px1,s_px0,s_pz1,s_pz0,s_qx1,s_qx0,s_qz1,s_qz0,
                                                  acoffx1,acoffx2,acoffz1,acoffz2,deta,epsilu,theta,fs,ds,zs,is,true);
                  s_u0=s_u1; s_w0=s_w1; s_px0=s_px1; s_pz0=s_pz1; s_qx0=s_qx1; s_qz0=s_qz1;   
            ...
            for(it=nt-1;it>=(int)(2/(favg*dt));it--)
            { 
                  ...
                  update_stress<<<(nnx*nnz+511)/512, 512>>>(nx,nz,nnx,nnz,dt,dx,dz,g_u1,g_w1,g_P,g_Q,ini_vp,npml,
		...
		//   Poynting_Adcigs<<<(nxa*nz+511)/512, 512>>>(nnz, nx, nz, npml, na, da, dcdp, adcigs, s_P, s_Q, s_u0, s_w0, g_P, g_Q, g_u0, g_w0);
            }//it loop end
